...
AGENTS.md — project-specific rules and scope
DECISIONS.md — why things are built the way they are
PLAN.md — current milestone, ordered task list, what's done vs remaining
tasks/lessons.md — mistakes made and patterns learned across sessions

If any file does not exist, say so and continue with what is available.
Do not create placeholder versions of missing files without EDIT_OK.
...
### In Progress
- [ ] Task description — [who is working on it: Brian / Claude Code / Codex]
...
What the app does today (actual working features only,
not planned, not partially built, not assumed)
Repository map: files touched in recent change-sets +
installed shadcn/ui components (CLI-initialized only)
Data model: current tables, RLS status per table,
...
EDIT_OK Protocol
Default mode is propose-only. You may read, summarize, and suggest.
You may edit files only when I include:
EDIT_OK: [file list or "any files needed for this task"]
Examples:
EDIT_OK: src/lib/auth.ts only
EDIT_OK: any files needed for this task
Without EDIT_OK: propose exact changes, label them, wait.
If I say READ-ONLY: inspect, summarize, and propose only. No exceptions.

Toolchain Roles
ToolRolePermission modelClaude.ai chatPlanning, architecture, prompt design, workflow decisionsN/A — advisory onlyClaude CodeCode execution, file edits, git operationsEDIT_OK required per sessionCodexAutonomous scoped tasks (docs, migrations, refactors)Scope defined in the prompt itself
Codex Rules

Codex tasks must be fully scoped in the prompt — there is no interactive
EDIT_OK loop. The prompt IS the scope.
Codex commits to a feature branch, never main, unless the prompt
explicitly says "push to main" for docs-only changes.
Codex must not touch files outside the declared scope.
If Codex and Claude Code could touch the same files in overlapping
sessions, coordinate through PLAN.md — one owner per file at a time.

Session Health
If Claude Code exhibits any of the following, stop the session immediately:

Repeats a mistake it already corrected earlier in the session
References a file or path that does not exist
Produces output that ignores the Session State block
Suggests changes to files outside the declared scope without flagging it
Generates code that contradicts a decision recorded in DECISIONS.md

When stopping: update PLAN.md with where you left off, then start a
new session. Do not try to course-correct in a degraded context window.

Branch Strategy
Default: work on a feature branch.
git checkout -b [type]/[short-description]
Branch naming follows commit types: feat/, fix/, chore/, refactor/,
security/, docs/, test/, ci/
Merge to main only after:

Gate command passes (see Quality Gates)
Diff-smell checklist passes (see docs/diff-smell-checklist.md)

Direct-to-main exceptions (all three must be true):

Docs-only change (no code, no config, no schema)
Single file touched
Does not affect auth, API routes, or RLS

Everything else goes through a branch + PR. Vercel auto-deploys main.
Main = production. Treat it that way.

Response Format (Every Time)

Session State block (4 lines)
One-line summary of what you are about to do
Files changed: list (max 5 unless I approve more)
File-by-file code blocks with full paths
What changed: 1-3 bullets describing what was actually done
...
If a decision gate is hit: STOP. State problem, 2-3 options,
recommendation, wait.
...
Beta Mode (When I Say "We Have Users" or "Beta Mode")
The app is in front of real people. Higher bar than v0, lower than production.

Error states on all user-facing flows (forms, uploads, API calls that
the user triggers). Silent failures are not acceptable.
Basic input validation on all write paths
Structured logging on every API route (route, duration, status, error).
Never log PII or PHI.
Auth paths must have at least one test (happy path + one failure case)
Mobile layout not required unless I say otherwise
Loading skeletons not required unless the wait exceeds 2 seconds
Sentry or equivalent must be configured. If it is not, flag it as a
blocker before writing feature code.
...
Assumption Budget
You may make up to 3 explicit assumptions to proceed in v0 or beta.
...
If any assumption is invalidated, the budget resets to 0 for that
change-set. Stop, get the correct answer, then re-scope.
...
A feature introduces a new paid API call per user action — flag the
estimated cost per call and monthly projection at current user count

When proposing a new dependency: state what it does, weekly npm downloads,
last publish date, and known vulnerabilities. If last publish > 12 months
or downloads < 10k/week, flag as risky.
...
Always-On Quality Gates (v0, Beta, and Production)
Gate Command
Default gate command:
pnpm lint && pnpm typecheck && pnpm test
If the project uses a different gate, it will be defined in AGENTS.md.
Gate must pass before any PR is opened. If gate is not yet configured
in the project, flag it as a blocker — do not skip it silently.
Git Discipline
Mandatory commits required when:

The task touches auth, schema, or RLS
You are in Beta Mode or Production Mode
...
Every migration must be tested against the dev database before applying
to production. Destructive migrations (DROP, ALTER column type, DELETE)
require a decision gate. Down migrations are recommended but not
required in v0 or beta.
...
A significant feature is any change that adds a new user-facing capability,
modifies the data model, or changes auth behavior. Refactors, bug fixes,
and docs updates are not significant features.